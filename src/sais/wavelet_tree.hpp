...
namespace sais
{
	...
	inline void splitByLSB(char16_t* ones, char16_t* zeros, const char16_t* data, size_t size)
	{
	...
	inline size_t writeMSBs(uint8_t* out, size_t bitOffset, const char16_t* data, size_t size)
	{
		...
		for (size_t i = 0; i < bodySize / rSize; ++i)
		{
			...
			for (size_t j = 0; j < rSize; ++j)
			{
				auto v = (size_t)((curData[j] & (1 << 15)) ? 1 : 0);
				g |= v << j;
			}
			((size_t*)out)[i + (headSize ? 1 : 0)] = g;
			oneCnt += popcount(g);
		...
		if (tailSize)
		{
			...
			for (size_t i = 0; i < tailSize; ++i)
			{
				auto v = (size_t)((data[headSize + bodySize + i] & (1 << 15)) ? 1 : 0);
				tail |= v << (i & (rSize - 1));
				oneCnt += v ? 1 : 0;
			}
		}
	...
	inline void splitByMSB(char16_t* ones, char16_t* zeros, const char16_t* data, size_t size)
	{
		for (size_t i = 0; i < size; ++i)
		{
			if ((*data & (1 << 15)))
	...
	class WaveletTree
	{
		...
		static constexpr size_t rSize = sizeof(size_t) * 8;
		static constexpr size_t depth = sizeof(char16_t) * 8;
		...
		template<class Fn>
		size_t enumerate(size_t i, char16_t c, size_t l, size_t r, size_t offsetIdx, Fn&& fn) const
		{
			size_t ret = 0;
	...
	public:
		...
		WaveletTree(const char16_t* data, size_t size)
		{
			const size_t alignedSize = (size + bitAlignmentSize - 1) & ~(bitAlignmentSize - 1);
			length = size;
			bits = std::unique_ptr<uint8_t[]>(new uint8_t[alignedSize * sizeof(char16_t)]);
			...
			std::vector<char16_t> buf(size * 2);
		...
		size_t rank(char16_t c, size_t l) const
		{
			const size_t alignedSize = (length + bitAlignmentSize - 1) & ~(bitAlignmentSize - 1);
			size_t offsetIdx = 0;
