...
namespace LCS
{
	...
	matrix<size_t> computeLCSMatrix(RandomIter xBegin, RandomIter xEnd, RandomIter yBegin, RandomIter yEnd)
	{
		...
		for (size_t i = 1; xBegin != xEnd; ++i, ++xBegin)
		{
			...
			{
				if (*xBegin == *yIt) c.at(i, j) = c.at(i - 1, j - 1) + 1;
	...
	template<class RandomIter>
	std::vector<typename std::iterator_traits<RandomIter>::value_type> _getLCS(const matrix<size_t>& c, RandomIter xBegin, RandomIter xEnd, RandomIter yBegin, RandomIter yEnd)
	{
	...
	template<class RandomIter>
	std::vector<std::pair<int, typename std::iterator_traits<RandomIter>::value_type>> _getDiff(const matrix<size_t>& c, RandomIter xBegin, RandomIter xEnd, RandomIter yBegin, RandomIter yEnd)
	{
		auto && dim = c.getDimension();
		auto i = dim.first - 1;
		auto j = dim.second - 1;
	...
	template<class RandomIter>
	std::vector<typename std::iterator_traits<RandomIter>::value_type> getLCS(RandomIter xBegin, RandomIter xEnd, RandomIter yBegin, RandomIter yEnd)
	{
		...
		while (*(xDiffEnd - 1) == *(yDiffEnd - 1) && xDiffBegin != xDiffEnd && yDiffBegin != yDiffEnd)
		{
			--xDiffEnd;
			--yDiffEnd;
		}

		auto c = computeLCSMatrix(xDiffBegin, xDiffEnd, yDiffBegin, yDiffEnd);
		auto lcs = _getLCS(c, xDiffBegin, xDiffEnd, yDiffBegin, yDiffEnd);
	...
	template<class RandomIter>
	std::vector<std::pair<int, typename std::iterator_traits<RandomIter>::value_type>> getDiff(RandomIter xBegin, RandomIter xEnd, RandomIter yBegin, RandomIter yEnd)
	{
		auto xDiffBegin = xBegin;
		auto yDiffBegin = yBegin;
		auto xDiffEnd = xEnd;
		...
		while (xDiffBegin != xDiffEnd && yDiffBegin != yDiffEnd && *(xDiffEnd - 1) == *(yDiffEnd - 1))
		{
			--xDiffEnd;
			--yDiffEnd;
		}
		...
		auto c = computeLCSMatrix(xDiffBegin, xDiffEnd, yDiffBegin, yDiffEnd);
		auto diff = _getDiff(c, xDiffBegin, xDiffEnd, yDiffBegin, yDiffEnd);
