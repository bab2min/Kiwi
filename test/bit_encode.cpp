...
TEST(BitEncoder, Variable_1_uint8)
{
	std::mt19937_64 rng;
	...
	for (auto len : lengthes)
	{
		...
		std::ostringstream oss;
		kiwi::lm::VariableLengthEncoder<std::ostringstream&, kiwi::lm::BitSeq<1, 4, 9, 15>, uint8_t> encoder{ oss };
		for (size_t i = 0; i < len; ++i)
		{
			encoder.write(orig[i]);
		}
		encoder.flush();

		kiwi::lm::VariableLengthDecoder<std::istringstream, kiwi::lm::BitSeq<1, 4, 9, 15>, uint8_t> decoder{ oss.str() };
		for (size_t i = 0; i < len; ++i)
		{
			reconstructed.emplace_back(decoder.read());
		}
...
TEST(BitEncoder, Variable_1_uint32)
{
	std::mt19937_64 rng;
	std::geometric_distribution<size_t> ud{ 0.005 };
	for (auto len : lengthes)
	{
		...
		std::ostringstream oss;
		kiwi::lm::VariableLengthEncoder<std::ostringstream&, kiwi::lm::BitSeq<1, 4, 9, 15>, uint32_t> encoder{ oss };
		for (size_t i = 0; i < len; ++i)
		{
			encoder.write(orig[i]);
		}
		...
		kiwi::lm::VariableLengthDecoder<std::istringstream, kiwi::lm::BitSeq<1, 4, 9, 15>, uint32_t> decoder{ oss.str() };
		for (size_t i = 0; i < len; ++i)
		{
			reconstructed.emplace_back(decoder.read());
		}
...
TEST(BitEncoder, Variable_2_uint8)
{
	std::mt19937_64 rng;
	std::geometric_distribution<size_t> ud{ 0.01 };
	for (auto len : lengthes)
	{
		std::vector<size_t> orig, reconstructed;
		for (size_t i = 0; i < len; ++i)
		...
		std::ostringstream oss;
		kiwi::lm::VariableLengthEncoder<std::ostringstream&, kiwi::lm::BitSeq<1, 3, 5, 7, 9, 11>, uint8_t> encoder{ oss };
		for (size_t i = 0; i < len; ++i)
		{
			encoder.write(orig[i]);
		}
		...
		kiwi::lm::VariableLengthDecoder<std::istringstream, kiwi::lm::BitSeq<1, 3, 5, 7, 9, 11>, uint8_t> decoder{ oss.str() };
		for (size_t i = 0; i < len; ++i)
		{
			reconstructed.emplace_back(decoder.read());
		}
...
TEST(BitEncoder, Variable_2_uint32)
{
	std::mt19937_64 rng;
	std::geometric_distribution<size_t> ud{ 0.01 };
	for (auto len : lengthes)
	{
		...
		std::ostringstream oss;
		kiwi::lm::VariableLengthEncoder<std::ostringstream&, kiwi::lm::BitSeq<1, 3, 5, 7, 9, 11>, uint32_t> encoder{ oss };
		for (size_t i = 0; i < len; ++i)
		{
			encoder.write(orig[i]);
		}
		...
		kiwi::lm::VariableLengthDecoder<std::istringstream, kiwi::lm::BitSeq<1, 3, 5, 7, 9, 11>, uint32_t> decoder{ oss.str() };
		for (size_t i = 0; i < len; ++i)
		{
			reconstructed.emplace_back(decoder.read());
		}
...
TEST(BitEncoder, Variable_3_uint8)
{
	std::mt19937_64 rng;
	std::geometric_distribution<size_t> ud{ 0.01 };
	for (auto len : lengthes)
	{
		...
		std::ostringstream oss;
		kiwi::lm::VariableLengthEncoder<std::ostringstream&, kiwi::lm::BitSeq<1, 4, 9, 16, 28>, uint8_t> encoder{ oss };
		for (size_t i = 0; i < len; ++i)
		{
			encoder.write(orig[i]);
		}
		...
		kiwi::lm::VariableLengthDecoder<std::istringstream, kiwi::lm::BitSeq<1, 4, 9, 16, 28>, uint8_t> decoder{ oss.str() };
		for (size_t i = 0; i < len; ++i)
		{
			reconstructed.emplace_back(decoder.read());
		}
...
TEST(BitEncoder, Variable_3_uint32)
{
	std::mt19937_64 rng;
	std::geometric_distribution<size_t> ud{ 0.01 };
	for (auto len : lengthes)
	{
		...
		std::ostringstream oss;
		kiwi::lm::VariableLengthEncoder<std::ostringstream&, kiwi::lm::BitSeq<1, 4, 9, 16, 28>, uint32_t> encoder{ oss };
		for (size_t i = 0; i < len; ++i)
		{
			encoder.write(orig[i]);
		}
		...
		kiwi::lm::VariableLengthDecoder<std::istringstream, kiwi::lm::BitSeq<1, 4, 9, 16, 28>, uint32_t> decoder{ oss.str() };
		for (size_t i = 0; i < len; ++i)
		{
			reconstructed.emplace_back(decoder.read());
		}
